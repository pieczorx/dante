...
int find_string(const char tab[], const char tab2[]) {
  ...
  for(int i = 0; i < my_strlen(tab) - my_strlen(tab2) + 1; i++) {
    index = i;
    int isGood = 1;
    for(int i2 = 0; i2 < my_strlen(tab2); i2++) {
      //printf("%c %c \n", tab[index + i2], tab2[i2]);
      if(tab[index + i2] != tab2[i2]) {
        isGood = 0;
      }
    }
    if(isGood) {
...
int rfind_string(const char tab[], const char tab2[]) {
  ...
  for(int i = 0; i < my_strlen(tab) - my_strlen(tab2) + 1; i++) {
    index = (my_strlen(tab) - my_strlen(tab2) + 1) - i - 1;
    int isGood = 1;
    for(int i2 = 0; i2 < my_strlen(tab2); i2++) {
      index2 = my_strlen(tab2) - i2 - 1;
      //printf("%c %c \n", tab[index + i2], tab2[i2]);
      if(tab[index + index2] != tab2[index2]) {
        isGood = 0;
      }
    }
    if(isGood) {
...
int main() {
  ...
  printf("%i\n%i", find_string(word, word2), rfind_string(word, word2));
